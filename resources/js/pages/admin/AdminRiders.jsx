...
function CreateRiderModal({ onClose, onCreated }) {
    ...
    const previewPassword = form.last_name
        ? `${form.last_name.toLowerCase()}${new Date().getFullYear()}`
        : null;
    ...
    if (created) return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
                ...
                <h2 className="font-bold text-slate-800 text-lg mb-1">Rider Created!</h2>
                <p className="text-slate-500 text-sm mb-5">Share these credentials with the rider.</p>

                <div className="bg-slate-50 rounded-xl p-4 text-left space-y-2 mb-5">
                    <div className="flex justify-between text-sm">
                        <span className="text-slate-400">Name</span>
                        <span className="font-semibold text-slate-700">{created.rider.name} {created.rider.last_name}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-slate-400">Email</span>
                        <span className="font-semibold text-slate-700">{created.rider.email}</span>
                    </div>
                ...
                <p className="text-xs text-slate-400 mb-4">Rider can change their password after logging in.</p>
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 my-4">
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="bg-slate-50 rounded-xl px-4 py-3 flex items-center justify-between">
                        <div>
                            <p className="text-xs font-semibold text-slate-500">Auto-generated Password</p>
                            <p className="text-xs text-slate-400 mt-0.5">Based on last name + current year</p>
                        </div>
                        <span className={`font-mono font-bold text-sm px-3 py-1.5 rounded-lg ${previewPassword ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-300'}`}>
                            {previewPassword || 'lastname2026'}
                        </span>
                    </div>
